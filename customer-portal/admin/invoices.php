<?php
/**
 * Customer Portal – admin/invoices.php
 * Factuurbeheer.
 */
...
if ($action === 'save_invoice' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['status'] === 'paid') {
        ...
        if (!$existing['paid_at']) {
            $db->update("{$p}cp_invoices", ['paid_at' => date('Y-m-d H:i:s')], ['id' => $id]);
        }
    }

    flash('success', 'Factuur opgeslagen.');
    redirect(BASE_URL . 'admin/?module=customer-portal&page=invoices&action=edit&id=' . $id);
}
...
if ($action === 'mark_sent' && isset($_GET['id'])) {
    ...
    redirect(BASE_URL . 'admin/?module=customer-portal&page=invoices');
}
...
if ($action === 'edit' && isset($_GET['id'])) {
    $invoice     = $db->fetch(
        "SELECT i.*, c.contact_name, c.company_name
         FROM `{$p}cp_invoices` i
         JOIN `{$p}cp_customers` c ON c.id = i.customer_id
         WHERE i.id = ?",
        [(int)$_GET['id']]
    );
    $invoiceItems = $db->fetchAll("SELECT * FROM `{$p}cp_invoice_items` WHERE `invoice_id` = ? ORDER BY `sort_order`", [(int)$_GET['id']]);
    if (!$invoice) {
        flash('error', 'Factuur niet gevonden.');
        redirect(BASE_URL . 'admin/?module=customer-portal&page=invoices');
    }
    ?>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0"><sl-icon name="receipt"></sl-icon> Factuur bewerken – <?= e($invoice['invoice_number']) ?></h1>
        <a href="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices" class="btn btn-outline-secondary">
    ...
    <form method="post" action="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices">
        <?= csrf_field() ?>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" value="<?= e($invoice['company_name'] ?: $invoice['contact_name']) ?>" disabled>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Status</label>
                        <select name="status" class="form-select">
                            <?php foreach (['concept' => 'Concept', 'sent' => 'Verzonden', 'paid' => 'Betaald', 'overdue' => 'Verlopen', 'cancelled' => 'Geannuleerd'] as $v => $l): ?>
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="title" class="form-control" value="<?= e($invoice['title']) ?>">
                    </div>
                    <div class="col-12">
                        <label class="form-label">Introductietekst</label>
                        <textarea name="intro" class="form-control" rows="3"><?= e($invoice['intro']) ?></textarea>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Factuurdatum</label>
                        <input type="date" name="invoice_date" class="form-control" value="<?= e($invoice['invoice_date']) ?>">
                    </div>
                    <div class="col-md-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="tax_rate" class="form-control" value="<?= e($invoice['tax_rate']) ?>" step="0.01">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Korting (€)</label>
                        <input type="number" name="discount" class="form-control" value="<?= e($invoice['discount']) ?>" step="0.01">
                    </div>
                    <div class="col-12">
                        <label class="form-label">Voetnoot</label>
                        <textarea name="footer" class="form-control" rows="2"><?= e($invoice['footer']) ?></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex gap-2">
            ...
            <a href="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices&action=mark_sent&id=<?= $invoice['id'] ?>&<?= csrf_token() ?>"
               class="btn btn-outline-info" onclick="return confirm('Factuur als verzonden markeren?')">
                <sl-icon name="send"></sl-icon> Markeer verzonden
            </a>
            <a href="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices&action=mark_paid&id=<?= $invoice['id'] ?>&<?= csrf_token() ?>"
               class="btn btn-outline-success" onclick="return confirm('Factuur als betaald markeren?')">
                <sl-icon name="check-circle"></sl-icon> Markeer betaald
            </a>
        </div>
    </form>
...
$statusLabels = ['concept' => ['Concept', 'secondary'], 'sent' => ['Verzonden', 'info'], 'paid' => ['Betaald', 'success'], 'overdue' => ['Verlopen', 'warning'], 'cancelled' => ['Geannuleerd', 'danger']];

// Totalen open facturen
...
?>
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0"><sl-icon name="receipt"></sl-icon> Facturen</h1>
    <button class="btn btn-primary" onclick="document.getElementById('createModal').style.display='flex'">
        <sl-icon name="plus-lg"></sl-icon> Nieuwe factuur
    </button>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-md-6">
        <div class="card text-center">
            <div class="card-body">
                <div class="fs-2 fw-bold text-warning">€ <?= number_format($openTotal['total'], 2, ',', '.') ?></div>
    ...
    <div class="col-md-6">
        <div class="card text-center">
            <div class="card-body">
                <div class="fs-2 fw-bold text-success">€ <?= number_format($paidTotal['total'], 2, ',', '.') ?></div>
        ...
        <table class="table table-hover mb-0">
            ...
            <?php else: ?>
                <?php foreach ($invoices as $inv):
                    [$label, $color] = $statusLabels[$inv['status']] ?? ['Onbekend', 'secondary'];
                ...
                <tr <?= $overdue ? 'class="table-warning"' : '' ?>>
                    ...
                    <td><span class="badge bg-<?= $color ?>"><?= $label ?><?= $overdue ? ' !' : '' ?></span></td>
                    <td><?= $inv['due_date'] ? date('d-m-Y', strtotime($inv['due_date'])) : '–' ?></td>
                    <td class="text-end">
                        <a href="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices&action=edit&id=<?= $inv['id'] ?>" class="btn btn-sm btn-outline-primary">
                            <sl-icon name="pencil"></sl-icon>
                        </a>
                        <a href="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices&action=delete&id=<?= $inv['id'] ?>&<?= csrf_token() ?>"
                           class="btn btn-sm btn-outline-danger"
                           onclick="return confirm('Factuur verwijderen?')">
    ...
    <div class="card" style="width:500px">
        ...
        <div class="card-body">
            <form method="post" action="<?= BASE_URL ?>admin/?module=customer-portal&page=invoices">
                ...
                <div class="mb-3">
                    <label class="form-label">Titel</label>
                    <input type="text" name="title" class="form-control" value="Factuur">
                </div>
                <div class="d-flex gap-2">
